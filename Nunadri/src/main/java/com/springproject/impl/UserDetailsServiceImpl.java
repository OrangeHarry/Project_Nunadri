...
@Service /*
			 * UserDetailsService : 데이터베이스에서 회원 정보를 가져오는 역할 loadUserByUsername() 메소드가 존재하며,
			 * 회원정보를 조회하여 사용자의 정보와 권한을 갖는 UserDetails 인터페이스를 반환 스프링 시큐리티에서
			 * UserDetailService를 구현하고 있는 클래스를 통해 로그인 기능을 구현
			 */
public class UserDetailsServiceImpl implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String id) throws UsernameNotFoundException {
		...
		if (member == null) {
			...
			SecurityUser user = new SecurityUser(member.getId(), member.getPwd(), member.getNickname(),
