...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity security) throws Exception {
		...
		security.authorizeRequests()
				.antMatchers("/", "/login", "/signup", "/findid", "/findpwd", "/login/error", "/checkPassword",
						"/checkPwdCode", "/checkMail", "/sendId")
				.permitAll().antMatchers("/admin/*").hasRole("ADMIN").anyRequest().authenticated();

		security.csrf().disable();

		security.formLogin().loginPage("/login").defaultSuccessUrl("/", true).usernameParameter("id")
				.passwordParameter("pwd").failureUrl("/login/error").and().logout()
				.logoutRequestMatcher(new AntPathRequestMatcher("/logout")).logoutSuccessUrl("/").and().oauth2Login()
				.loginPage("/login") // 소셜 로그인된뒤의 후처리가 필요함
